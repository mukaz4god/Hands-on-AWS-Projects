...
This project implements secure object storage using Amazon S3 with the following controls:
...
LAB 1 – Create Secure S3 Buckets
...
This ensures access is controlled only through IAM policies and bucket policies, which is an AWS security best practice.

Block Public Access

Ensure all options remain enabled:
...
LAB 2 – Enable Encryption

Encryption protects sensitive data stored in S3.
...
or create a customer-managed KMS key.
...
LAB 3 – Configure Bucket Policies & Access Controls

Bucket policies define who can access objects in the bucket.
...
Example: Allow read access only from a specific IAM role.
...
Versioning protects against accidental deletion or overwriting.
...
Enable logging and select:

Target bucket → company-s3-access-logs
...
After completing this project you have successfully implemented:
...
This setup aligns with AWS security best practices and the AWS Well-Architected Framework Security Pillar.
